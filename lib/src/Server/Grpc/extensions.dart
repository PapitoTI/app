...
extension TouristGrpc on Tourist {
  TouristModel toModel() =>
      TouristModel(this.profileImageUrl, this.name, this.email, this.phone);
}

extension GuideGrpc on Guide {
  GuideModel toModel() => GuideModel(this.profileImageUrl, this.name,
      this.email, this.phone, this.certificate, this.accountBalance.toString());
}

extension SpotGrpc on Spot {
  SpotModel toModel() => SpotModel(
      this.name, this.address, this.category, this.description, this.imageUrls);
}

extension ScheduleGrpc on Schedule {
  ScheduleModel toModel() => ScheduleModel(
      this.tour.toModel(),
      this.tourist.toModel(),
      DateFormat.yMd().add_Hms().parse(this.date),
      ScheduleStatus.values[this.status]);
}
